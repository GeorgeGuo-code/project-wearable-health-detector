...
module sm_main (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            btn_mode_s2_d1    <= 1'b1;     // 对齐按钮取反后的常态(1=未按下)
            btn_confirm_s2_d1 <= 1'b1;
        end else begin
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            debounce_cnt        <= 21'd0;
            debouncing          <= 1'b0;
            btn_mode_stable     <= 1'b1;     // 对齐按钮取反后的常态
            btn_confirm_stable  <= 1'b1;
        end else begin
            // 检测信号自身跳变 → 启动消抖
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            btn_mode_stable_d1    <= 1'b1;     // 对齐按钮常态=1, 避免复位假边沿
            btn_confirm_stable_d1 <= 1'b1;
        end else begin
