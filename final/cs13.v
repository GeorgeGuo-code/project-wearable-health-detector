...
(
    ...
    // ---- Flash 操作提示 (本期新增, 优先级高于 flash_view_en) ----
    input wire        flash_op_en,      // 1 = 显示 flash 操作提示页 (覆盖 FLASH 视图)
    ...
    //   优先级: flash_op_en > flash_view_en > work_en 派生的 display_mode
    wire [2:0] scr;
    assign scr = flash_op_en  ? 3'd3 :
                 flash_view_en ? 3'd7 :
                 (work_en ? (display_mode + 3'd1) : 3'd0);
    ...
    // cadence(spm) → 实际步频(sps×10): cad/6
    wire [7:0] cad_freq = flash_avg_cad / 8'd6;
    wire [7:0] fcd_d1 = d2a(cad_freq / 8'd10);
    wire [7:0] fcd_d2 = d2a(cad_freq % 8'd10);
    ...
    // 实时步频: cadence(spm)/6 = sps×10
    wire [7:0] rt_freq   = cadence / 8'd6;
    wire [7:0] cd_d1 = d2a(rt_freq / 8'd10);
    wire [7:0] cd_d2 = d2a(rt_freq % 8'd10);
    ...
    function [39:0] font_rom;
        ...
        begin
            case (ch)
                ...
                8'd  3: font_rom = 40'h70_F8_FC_F8_70;  // ♥ heart
    ...
    always @(posedge clk or negedge rst or negedge en) begin
        if (!rst || !en) begin
            ...
            case (state)
                ...
                MAIN: begin
                    // cnt_main 循环 1..8: 显式 scr 填 1..4, default 清 b4..b7
                    // (否则 OLED b4..b7 永远不写, 显示上电 RAM 垃圾 -> 雪花)
                    ...
                    if (cnt_main == 5'd0) state <= INIT;
                        ...
                        {3'd1,5'd3}: begin y_p<=8'hb2; char<={"  ","S","P","S",":",cd_d1,".",cd_d2," S","T",":",st_h,st_t,st_u,st_v,"   "}; state<=SCAN; end
                        ...
                        //========== BLANK / DEFAULT ==========
                        // 修正 y_p: cnt_main=1 -> b0, ..., cnt_main=8 -> b7
                        // 旧公式 8'hb0|cnt_main[2:0] 在 cnt_main=8 时错位到 b0
                        default: begin y_p <= 8'hb0 + cnt_main - 5'd1; char <= {"  ",{16{8'h20}},"   "}; state <= SCAN; end
                        //========== SCREEN 3: FLASH OP PROMPT / FEEDBACK ==========
                        //   flash_op_message 0=Saving 1=Saved OK 2=Delete?/Error 3=Deleted
                        {3'd3,5'd1}: begin
                            y_p <= 8'hb0;
                            case (flash_op_message)
                                2'd0: char <= {"  ","  ","S","a","v","i","n","g",".",".","."," "," "," "," ","   "};
                                2'd1: char <= {"  ","  ","S","a","v","e","d"," "," ","O","K","!"," "," "," ","   "};
                                2'd2: char <= {"  ","  ","D","e","l","e","t","e","?"," "," "," "," "," "," ","   "};
                                2'd3: char <= {"  ","  ","D","e","l","e","t","e","d","!"," "," "," "," "," ","   "};
                            endcase
                            state <= SCAN;
                        end
                        {3'd3,5'd2}: begin
                            y_p <= 8'hb1;
                            case (flash_op_message)
                                2'd2: char <= {"  ","  ","P","r","e","s","s"," ","C","O","N","F","I","R","M","   "};
                                default: char <= {"  ",{16{8'h20}},"   "};
                            endcase
                            state <= SCAN;
                        end
                        {3'd3,5'd3}: begin y_p <= 8'hb2; char <= {"  ",{16{8'h20}},"   "}; state <= SCAN; end
                        {3'd3,5'd4}: begin y_p <= 8'hb3; char <= {"  ",{16{8'h20}},"   "}; state <= SCAN; end
                        //========== SCREEN 7: FLASH VIEW ==========
                        {3'd7,5'd1}: begin y_p<=8'hb0; char<={"  ","*"," ","F","L","A","S","H"," ","S","T","A","T","S","      "}; state<=SCAN; end
                        {3'd7,5'd2}: begin y_p<=8'hb1;
                            if (flash_step == 16'hDEAD)
                                char <= {"  ","      ","~","N","o"," ","D","a","t","a","~","      "};
                            else
                                char <= {"  ","S","T","E","P","=",fs_d1,fs_d2,fs_d3,fs_d4,"          "};
                            state<=SCAN; end
                        {3'd7,5'd3}: begin y_p<=8'hb2;
                            if (flash_step == 16'hDEAD)
                                char <= {"  "," "," "," "," "," "," "," "," "," "," "," "," "," "," "};
                            else
                                char <= {"  ","A","V","G","_","S","P","S","=",fcd_d1,".",fcd_d2,"        "};
                            state<=SCAN; end
                        {3'd7,5'd4}: begin y_p<=8'hb3;
                            if (flash_step == 16'hDEAD)
                                char <= {"  "," "," "," "," "," "," "," "," "," "," "," "," "," "," "};
                            else
                                char <= {"  ","A","V","G","_","H","R","=",fhr_d1,fhr_d2,"          "};
                            state<=SCAN; end
                    endcase
                end
