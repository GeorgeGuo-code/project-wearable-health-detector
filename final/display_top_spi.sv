...
module display_top_spi (
    ...
    // ---- Flash 操作提示 (新增, 透传给 cs13) ----
    input  wire        flash_op_en,
    input  wire [1:0]  flash_op_message
);
    ...
    // ---- LED: bit0=绿 bit1=黄 bit2=红 ----
    reg [2:0] led; reg [16:0] blk;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin led<=3'b001; blk<=17'd0; end        // 初始绿灯
        else begin
            blk<=blk+17'd1;
            if (!work_en) led<=blk[16]?3'b000:3'b001;          // 待机: 绿灯闪烁
            else case(health_status_eff)
                2'b00: led<=3'b001;  // 绿灯 (健康)
                2'b01: led<=3'b010;  // 黄灯 (中等)
                2'b10: led<=3'b100;  // 红灯 (危险)
                default:led<=3'b001;
            endcase
        end
    end
